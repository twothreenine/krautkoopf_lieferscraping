...
def read_config(supplier, subconfig=""):
    ...
    if subconfig:
        if subconfig not in supplier_config:
...
foodsoft_config = {'foodsoft_url': os.environ['TR_FOODSOFT_URL'],'foodsoft_user': os.environ['TR_FOODSOFT_USER'],'foodsoft_password': os.environ['TR_FOODSOFT_PASS']}
...
def compare_string(article, article_from_last_run, article_from_foodsoft, string_type, supplier_config, manual_changes, notifications):
    ...
    elif getattr(article, string_type) != getattr(article_from_foodsoft, string_type):
        replace = False
        if getattr(article, string_type) == getattr(article_from_last_run, string_type):
            replace = True
            replaced_string = getattr(article, string_type)
            manual_string = getattr(article_from_foodsoft, string_type)
            if article.order_number not in supplier_config["manual_changes"]:
                supplier_config["manual_changes"][article.order_number] = {}
            supplier_config["manual_changes"][article.order_number][string_type] = {}
            supplier_config["manual_changes"][article.order_number][string_type]["replaced"] = replaced_string
            supplier_config["manual_changes"][article.order_number][string_type]["manual"] = manual_string
            notifications.append("Keeping manual change of article " + string_type + ": " + replaced_string + " -> " + manual_string)
        elif string_type in manual_changes:
...
def compare_manual_changes(articles, supplier, supplier_id, notifications=[], compare_name=True, compare_note=True, compare_manufacturer=True, compare_origin=True, compare_unit=True, compare_price=True, compare_vat=False, compare_deposit=False, compare_unit_quantity=False):
    ...
    supplier_config = read_config(supplier=supplier, subconfig="manual_changes")

    # Connect to your Foodsoft instance and download the articles CSV of the supplier
    fsc = FSConnector(url=foodsoft_config['foodsoft_url'], supplier_id=supplier_id, user=foodsoft_config['foodsoft_user'], password=foodsoft_config['foodsoft_password'])
    ...
    for article in articles:
        ...
        manual_changes = {}
        if article.order_number in supplier_config["manual_changes"]:
            manual_changes = supplier_config["manual_changes"][article.order_number]

        if compare_name:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "name", supplier_config, manual_changes, notifications)
        if compare_note:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "note", supplier_config, manual_changes, notifications)
        if compare_manufacturer:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "manufacturer", supplier_config, manual_changes, notifications)
        if compare_origin:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "origin", supplier_config, manual_changes, notifications)
        if compare_unit:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "unit", supplier_config, manual_changes, notifications)
        if compare_price:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "price_net", supplier_config, manual_changes, notifications)
        if compare_vat:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "vat", supplier_config, manual_changes, notifications)
        if compare_deposit:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "deposit", supplier_config, manual_changes, notifications)
        if compare_unit_quantity:
            supplier_config = compare_string(article, article_from_last_run, article_from_foodsoft, "unit_quantity", supplier_config, manual_changes, notifications)
...
def compose_message(supplier, supplier_id, categories, ignored_categories, ignored_subcategories, ignored_articles, notifications):
    ...
    text += foodsoft_config["foodsoft_url"] + "suppliers/" + str(supplier_id) + "/articles/upload"
...
def write_csv(supplier, articles, notifications=[]):
    rows = get_data_from_articles(articles=articles, notifications=notifications)
