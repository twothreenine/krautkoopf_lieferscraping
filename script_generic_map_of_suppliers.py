...
def config_variables(): # List of the special config variables this script uses, whether they are required and how they could look like
    return [
        ...
        base.Variable(name="style by supplier category", required=False, example={'category 1': {'icon': 'leaf', 'icon-prefix': '', 'icon-color': ''}}),
        ...
        # base.Variable(name="other supplier field(s)", required=False, example=[{'foodsoft field(s)': ['custom_fields_public_description'], 'label': ''}, {'foodsoft field(s)': ['custom_fields_ordering_cycle'], 'label': 'Ordering cycle'}], description="Other supplier info which should be added, optionally under the specified label"),
        ]
...
class ScriptRun(base.Run):
    ...
    def run_script(self, session):
        ...
        suppliers = session.foodsoft_connector.get_supplier_data(exclude_categories=base.read_in_config(config, "exclude supplier categories", []))
        ...
        for fc_address in fc_addresses_dict.keys():
            ...
            if location:
                ...
                supplier = foodsoft.Supplier(name=fc_address, address=address, description=fc_addresses_dict[fc_address].get("description"), latitude=location.latitude, longitude=location.longitude, \
        ...
        for supplier in suppliers:
            popup = folium.Popup(folium.Html(popup_html(supplier, show_category=True), script=True, width=200))
...
def popup_html(supplier, show_category=False):
    html = f"<h4>{supplier.name}</h4><p>📍 {supplier.address}</p>"
    if supplier.category and show_category:
        html += f"<p>🏷 {supplier.category}</p>"
    if supplier.description:
        html += f"<p>{supplier.description}</p>"
    return html
