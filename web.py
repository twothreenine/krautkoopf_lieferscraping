...
app = App()
plugin = bottle_session.SessionPlugin(cookie_lifetime=600)#, cookie_secure=True, cookie_httponly=True)
app.install(plugin)
...
def check_login(session, submitted_form, instance):
    global app

    if app.foodsoft_connector:
        if session.get('instance') == instance:
            return True
        else:
            return False
...
@app.route('/<fc>')
def login(session, fc):
    if check_login(session, bottle.request.forms, fc):
        return main_page()
    else:
        return login_page(fc)

@app.route('/<fc>', method='POST')
def do_main(session, fc):
    submitted_form = bottle.request.forms
    if 'logout' in submitted_form:
        ...
        if request_path:
            ...
            return root_page()
    elif check_login(session, submitted_form, fc):
...
@app.route('/<fc>/<configuration>', method='ANY')
def configuration(session, fc, configuration):
    global app
    submitted_form = bottle.request.forms
    if check_login(session, submitted_form, fc):
...
@app.route('/<fc>/<configuration>/display/<run_name>', method='ANY')
def display_run(session, fc, configuration, run_name):
    global app
    submitted_form = bottle.request.forms
    if check_login(session, submitted_form, fc):
        importlib.invalidate_caches()
...
@app.route('/<fc>/<configuration>/new_run', method='ANY')
def start_script_run(session, fc, configuration):
    submitted_form = bottle.request.forms
    if check_login(session, submitted_form, fc):
        importlib.invalidate_caches()
        script = import_script(configuration=configuration)
...
@app.route('/<fc>/<configuration>/edit', method='ANY')
def edit_configuration(session, fc, configuration):
    submitted_form = bottle.request.forms
    if check_login(session, submitted_form, fc):
...
@app.route('/<fc>/<configuration>/delete', method='ANY')
def delete_configuration(session, fc, configuration):
    submitted_form = bottle.request.forms
    if check_login(session, submitted_form, fc):
...
@app.route('/download/<filename:path>', method='ANY')
...
def send_css(session, filename='styles.css'):
